...
| 2026-08-30 | R8-T3 抽检回执 | M1b 漏报试点（转正记账） | 12 段抽检包（阳性 3+边缘 5+阴性对照 4，覆盖全部非阴性判段）呈报后**用户回执「T3」=无异议**，无误标指出——**阳性 1.18%（含边缘 3.15%）升级为可引用基线**；折扣声明第 3 条（用户抽检）关闭，余留折扣=AI 自评初标+小样（引用时须带） | 数字可引用；**升格常设基线=仍待用户另行裁决**（报告 §6 建议：表外变体三类走校准批次流程，不直接扩表） | 无处置 | `docs/knowledge/redteam/missrate-pilot.md` §抽检回执 + `tasks/README.md` R8 收口 |
