...
- `writer_ab.jsonl`：10 组 Main + Skill 与隔离 Writer Agent 的匿名对照任务。
- `context_builder_ab.jsonl`：10 组合法的跨卷/多线或冲突事实任务，对照 Memory Skill 与 Context Builder；简单任务的禁止 Spawn 由 Agent 工作流负向测试覆盖。
...
4. Review Agent 必须是新的隔离 run；`review.record` 以 `review_subject` 登记 Receipt，并把结构化分数和判断保存为不可变 assessment Resource。
