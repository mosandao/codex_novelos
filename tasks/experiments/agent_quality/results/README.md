...
- `assessments/<case_id>.json`：Review Agent 生成并由 Review Receipt 绑定的结构化评分、blocking 与边界/冲突/winner 判断。
...
Reviewer 不得读取 `execution_manifest.jsonl` 中 A/B 标签对应的执行模式。评审完成后，汇总器才使用该映射解盲并计算 Writer 与 Context Builder 决策。
