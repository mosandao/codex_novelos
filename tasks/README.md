...
- [x] `DONE` R4 · 连续性账本直写通道缺口：chapter_facts 等六账本的候选晋升/落库曾无门工具覆盖——随 `plugin/` 退役（2026-08-27）已解决：写路径 = 主控 node:sqlite 受控直写（AGENTS.md「数据库访问」节），六账本落库由主控按 sql-reference.md 模板直写
...
- 门规格提取子代理运行中 → 产出 `docs/r2-js-gate-spec.md`（create_project.py 全语义规格 + 其余写库脚本清单 + JS 漂移风险点），落地后开始门实现。（该规格文档已随插件退役于 2026-08-28 删除）
...
- [x] `DONE` R5 签名链 prepare(`5c18305`,**生产库零写入**):creator-signature schema v3(version [1,2,3]+style_dna tier A-D+measured_features 按 fpr: 契约/source_ref 必填/tier=D→空 allOf)+project-create style_seed 可选段+migration 020 草案(照 018 模板,**未执行**;副本库实测 30 行零损失+CHECK 双向生效)+`scripts/novelos-import-personas.mjs`(dry-run 默认/--commit 硬编码拒绝生产库路径;副本 --commit 60 行 content_hash 全对)+personas-alias-map(刘慈欣 5 变体归并等 7 命中+trim 1)+fusion prompt「量体」四节(主干零改动)+`u5-u7-signature-package.md` 呈报包(备份/影响面/选样 15 卡含女频 2/回滚/schema.sql 再导出步骤)。**验证:guardrails 296/compose 28/fingerprint 49/verify 15/canary PASS/render 48+schema 迷你校验 17/17(v1/v2 兼容)**。执行轮遗留:composer validateFusionPayloadStruct 白名单随 style_seed 三联动放开/渲染器风格 DNA 段/v3 是否强制 style_dna/测试内核先行——全部随 U5-U7 裁决后执行
- [x] `DONE` R6 演练准备(`1a0d770`):`scripts/novelos-drill-prepare.mjs`(S0 隔离:checkpoint(TRUNCATE)+备份+VACUUM INTO+双方 sha256/wal/迁移版本/列清单+零污染证明+S1 冒烟清单;--source 必填无默认生产路径;/tmp 假库全流程验证 zero_pollution=true)+`tasks/r5-plans/r6-drill-book.md`(S0-S6 红方任务书:参照非 Canon/style_dna 非对账源/技巧名词不渗入正文三专项+U11 未落时 S4「接受跳过」替代口径+收口报告结构)+metrics/fixtures R6 骨架节。**验证:五套测试全绿(guardrails 296)**。**R6 正式执行等 U11(019)+U-dirs(方向选定)**
- [x] `DONE` 项目清理(用户指令「清理掉大运这个小说项目」,2026-08-29):受控事务删除 projects 1/bindings 1/planning-cross-check reviews 3——**books/characters/worlds/planning_assets 本为空,resources(211,内容寻址)与 creator_profiles(45,含 15 style_seed)零触碰**;附带清理:投影目录(全角冒号名)/drill 副本/该项目 compositions;三候选全文锚点归档进 `r6-s2-direction-candidates.md`(persona/genre 为库内资产,新项目可复用候选骨架)。**终态:projects=0 bindings=0 reviews=0 resources=211 profiles=45 migrations=v20;备份保留 `data/novelos-v2.db.bak-drill-20260829134535`**。R6 演练影响:基底项目不存在,S2 候选待新项目创建后按新 setup 重验复用(或走项目创建向导重建)
- [x] `DONE` R6 演练关闭·U-dirs 一并关闭(用户指令 2026-08-29「R6 关闭,待定项关闭」):S0-S6 演练不再执行,`novelos-drill-prepare.mjs`/`r6-drill-book.md` 保留归档;U-dirs 失去触发场景——三候选(1《大运还贷人》债务轴/2《失控点》异化轴·agent 推荐/3《散人名单》成神轴)锚点保留于 `r6-s2-direction-candidates.md` 待未来新项目复用;R3/R4 的「待 R6」悬挂项与 metrics M4/M6 占位一并失效。对抗审查三待定项(=lite 快档/SQLite 红线降格+FTS5/运行时拉取)全部关闭维持现状。详见文末「R6 关闭与待定项关闭裁决记录」
- [ ] `TODO` R5 签名链(migration 020/schema v3/personas 试点,前置 U5/U6/U7);校准批次(F-5/F-6 规则表扩项+F-8 code 透传)。(R6 全链路演练已关闭——用户裁决 2026-08-29,不再列待办)
- [ ] `TODO(呈报待批)` **R7 机器强制层复活与对抗审查修正**——任务书 `tasks/R7-gate-revival.md` v1(2026-08-29 落盘):T0 预检→T1 A1 空回执默认 FATAL+A7a/b 快修→T2 防漂移守护 JS 化(G-recipe+catalog manifest lint)→T3 写门 CLI `novelos-gate.mjs` 六子命令(旧 159 用例移植 ≥40 例验收集)→T4 promise_events+Claremont(021 落生产库含独立裁决门)→T5 语态槽→T6 收口。范围=修正案 A1/A2/A3/A4/A7;A5/A6/A8/A9/A10 缓行另行呈报;三待定项已裁决关闭不含。**未获用户「执行」指令不动代码与数据库**
- [x] `DONE` **R7 执行收口**(用户指令「执行」,2026-08-29):**T0** 备份 `data/novelos-v2.db.bak-r7-20260829234614`+六套基线(guardrails 296/fingerprint 49/verify 15/render 48/canary PASS/compose 17✓+11 FAIL 全环境性——T0 手工数 6 系截断误差,修正基线=11,生产库 projects=0 所致零回归)。**T1(A1+A7a/b)** `novelos-verify-review-evidence.mjs` v1.1.0:空 findings+approved 从 --strict-only 升**默认 FATAL**(P4-1 关闭),新增 `--allow-empty` 显式豁免留痕;幽灵指针修复(prompt.md:37 scene-dialogue/scene-fight-craft 实在 `catalog/skills/writing/`);`agent-recipes.json` 删 test_recipe_matrix.py 陈旧引用;`tests/__pycache__` 35 .pyc 尸体清除;接线三处(AGENTS.md:71/novel-review:24/novel-writing:7 步)。**T2(A7c)** guardrails 增 G2d/e/f(divergence/decision_scope 全等+词表合法,复活被删 test_recipe_matrix 守护)+G3(catalog manifest 常驻复核);新脚本 `novelos-catalog-manifest.mjs`(351 文件 sha256 逐字登记 `config/catalog-manifest.json`,--check 红绿双向实测);**378→379 全绿**。**T3(A2)** `scripts/novelos-gate.mjs` 六子命令(commit-review/lock-asset/accept-chapter/propagate-stale/validate-asset/register-characters):dry-run 默认+`--commit`+生产库须 `--allow-production`;GateFail=阻断零写入;语义源=27d34a4/da9ee5c TS 逐段移植+WP5 记账重写(state-machine.ts 从未入库——P2-5 再添实证)+8af69a8^ 七件 py 校验器常量逐字(偏离声明:jsonschema→必需字段检查);P4-2 reviewer_profile 前缀机器强制。`test-gate.mjs` **58/58**(propagate 10+register 12+状态机/回执门 23+CLI/validate/Claremont 13);生产库六子命令只读冒烟 data_version 前后一致零污染;口径更新 sql-reference.md「R7 机器门通道」节+AGENTS.md 第四约定。**T4(A3)** `db/migrations/021_promise_events.sql`(promise_events 追加表 plant/progress/twist/resolve/break+narrative_promises.resolved_chapter_id;存量不回填照 019 先例);副本验证 25→26 表零行损失+CHECK 拦截+双路径形状一致;schema.sql v21 同步再导出;sql-reference.md 连续性节补流水 SQL 模板;**021 落生产库已执行**(用户裁决「确认」2026-08-30:应用前备份 `data/novelos-v2.db.bak-r7-021-20260830142121`,单事务 021+版本登记 21,25→26 表行数零损失,integrity_check ok/foreign_key_check 违例 0,与 schema.sql v21 形状一致)。**T5(A4)** composer `prev_chapter_tail` 语态槽(最近 accepted 章结尾 800 字,不足整段注入注明):**渲染位=craft 卡之后自检节之前(生成点前最近端)**,首章缺位显式降级;SLOT_REGISTRY+manifest+recipes+slot_vocabulary 四方同步;端到端冒烟 exit 0 语态节 540 行紧邻自检 549 行。**T6 终值:七套全绿 guardrails 379/fingerprint 49/verify 15/render 48/canary PASS/gate 58/compose 17+11 环境性(与基线一致)**。M5 语态槽增量+M2 口径见 metrics 追加行;修正案状态回写 `docs/novelos-adversarial-cross-exam.md` §5
...
## R6 关闭与待定项关闭裁决记录（2026-08-29 · 用户指令：「R6 关闭，待定项关闭」）

- **R6 全链路演练关闭**：S0-S6 演练不再执行；`scripts/novelos-drill-prepare.mjs` 与 `tasks/r5-plans/r6-drill-book.md` 保留归档、不再排期。metrics **M4（收敛轮数）/ M6（红方修复率）/ R6 S0 全指标采集时点**随之关闭停止采集（`docs/knowledge/metrics.md` 已追加关闭行；原「待 R6」占位行按追加纪律作废留档不删）。
- **「待 R6」悬挂项一并失效**：R3 技巧名词不渗入正文盲测实证、R4 后续批次（book_summaries 深挖/plot 扩展）不再由 R6 反馈触发，是否重启由用户另行裁决。
- **U-dirs（S2 方向选定）随之关闭**：三候选（1《大运还贷人》债务轴/2《失控点》异化轴·agent 推荐/3《散人名单》成神轴）全文锚点保留归档于 `r6-s2-direction-candidates.md`，未来新项目创建后可复用；U11 无消费者（019/020 已落生产库，版本链 v20，无遗留）。
- **对抗审查三个待定项全部关闭、维持现状**（`docs/novelos-adversarial-cross-exam.md` §4）：①八级依赖链**不加** lite 快档；②「SQLite 权威库不可倒退」红线**维持原表述**，不做 FTS5 检索增量；③运行时拉取**不采纳**，维持机器组装+被动注入（按需 Read 指针腐坏作为缺陷记录在案，是否修复随 A7 修正案另行裁决）。
- **未决事项留痕**：修正案 A1-A10（同报告 §5）维持未裁决呈报状态，未获逐条裁决不执行；两份调研报告（`docs/novel-skills-cross-analysis.md` / `docs/novelos-adversarial-cross-exam.md`）暂为未跟踪文件未入库。
