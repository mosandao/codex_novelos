...
- [ ] `IN PROGRESS` R3 写作层知识:蒸馏首批 262 条(对话 65/节奏 151/开篇 46,裁-6 口径;源=data/knowledge/techniques.json gitignore,产物入 git 不含原文长引)→craft 卡模块(≤2560B/卡)+distilled.<domain>.json(placement card|slot 互斥)+knowledge: 槽机制(composer resolveSlots 分支/预算 4096B/top-5×2/512B 条)+scene-maps.json+prose-blindtest 配方(裁-3,slots=[subject]+manifest craft_refs)+guardrails 例文「非成稿标准」机器校验
- [ ] `TODO` R4 规划层参照(modules 预组合);R5 签名链(migration 020/schema v3/personas 试点,前置 U5/U6/U7);R6 全链路演练(前置 U11/U-dirs);校准批次(F-5/F-6 规则表扩项+F-8 code 透传)
