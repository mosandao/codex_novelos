...
- [ ] `BLOCKED(用户裁决)` **裁决包呈报**:U2 双模式边界确认(已按编号白名单实现)/U3 C 级锚点 warning 确认(已实现,实质收紧)/U4 注入预算确认(槽 4096B+卡面 43,836B 现值)/U5 schema 020 迁移执行+备份时点(副本库已验证 30 行零损失)/U6 personas 15 卡试点(选样名单在 `u5-u7-signature-package.md`)/U7 B 级语料授权/U11 019 迁移落生产库/U-dirs 演练方向——**R5 执行(备份→020→schema.sql 再导出→personas commit→三联动放开)与 R6 正式演练在此等待**
