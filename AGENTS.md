...
写库三约定：① ID 格式 `类型:uuid`；② resources.content 经 BLOB 写入并同步 content_hash（`'sha256:'+hex`，node:crypto 计算）；③ 多表写入单事务，任一步失败整体回滚。**第四约定**：关键状态写入优先走机器门 `scripts/novelos-gate.mjs`（commit-review/lock-asset/accept-chapter/propagate-stale/register-characters/validate-asset/open-adjudication/resolve-adjudication；dry-run 默认，写库须 `--commit`+生产库 `--allow-production`；GateFail=阻断零写入），门未覆盖的幂等读改才走受控 SQL 直写。

**读路径**：agent 查库用一次性 node:sqlite 只读查询；人类浏览用任意 SQLite 工具只读打开 `data/novelos-v2.db`（sqlite3 CLI 等第三方工具默认 `foreign_keys=OFF`——人工操作前先手动 `PRAGMA foreign_keys=ON`，否则可造孤儿行），或打开 `node scripts/novelos-render-projection.mjs --project <id> --verify` 渲染出的 `novels/` 投影目录阅读。Python MCP 通道、legacy-python 校验门与 DSH 插件均已删除，不要再寻找或重建它们。
...
3. `$novel-review` 审查：blocking+warning 必修，修复 = 新 revision 受控重组装；3 轮未收敛或同因复发 → 升级用户裁决（升级须过门落裁决单：`novelos-gate.mjs open-adjudication`，open 期间该 subject 的 lock/accept 门互锁阻断、下游经 `open_adjudications` 注入槽可见；裁决后 `resolve-adjudication` 解除）；同 subject 回执 ≥3 条且无裁决单时 lock/accept 被升级门硬阻断（findings 须带 `issue_key` 稳定键供「同因复发」机器比对），禁止无限打转。回执落库前跑 `node scripts/novelos-verify-review-evidence.mjs --receipt <回执> --draft <该版草稿>` 引文验证，FATAL（excerpt 无命中/缺失/subject_hash 错配/空查回执——**note 级凑数也算空查** 默认拦截，确需放行加 `--allow-empty` 并留痕）即打回、不得落库；落库走 `commit-review` 并传 `--writer-profile <写作端模型>`（防共谋机器强制：与审查端同模型 GateFail，确需豁免 `--allow-same-provider` 留痕）。
