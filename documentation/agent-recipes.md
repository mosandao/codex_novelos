...
|---|---|---|---|---|---|
| fusion（onboarding/creator-signature-fusion） | kernel_full, selected_archetypes, archetype_roster, persona_hints, project_setup, persona_fingerprints | expansive | flag | creator_derivation_candidate（jsonschema 信封 + 签名 v2 深层校验；v3 带 kernel_origin） | 错配警告 → 呈报用户裁决，未获裁决不落库；候选解析失败要求融合智能体重出，主控禁手工改写 |
...
| volume_outline（planning/volume-outline） | upstream:story_arc | balanced | propose_only | 卷纲候选（节奏密度约束） | 修复循环 |
