...
- 项目创建向导已接通：以 Task 09 的“系统叙事原型 + 项目化最小派生”替代了直接新建与复用；V3 要求 `derive` 作者签名模式，页面按项目约束确定性推荐 Top 3 系统原型。单原型经 `project.wizard.reconcile_archetypes` 确定性融合（`parent_source:"scored"`）；多原型（≥2）由临时 `onboarding_agent` 做 LLM 深度融合，再把 Agent 判定的 parent 与完整签名作为 `fused_parent_version_id` / `fused_signature` 传给 reconcile 确定性收口（`parent_source:"fused"`），由 MCP 自动折算 overrides diff（Task 12，Task 24）。MCP 在同一事务中创建项目与精确 Creator Profile revision/Hash 绑定；二级方向仍是随一级题材切换的静态 LLM 预生成候选。落库事务不执行运行时 LLM 生成，LLM 只在 `onboarding_agent` 的 Codex run 内运行；也不创建规划资产；本地 `file://` 页面可以生成 JSON，但不直接写入数据库。
