...
| `NOVELOS_AGENT_CONTRACT_PATH` | `novelos_mcp.server` | 自动发现；生产脚本显式使用 `config/agents.yaml` | 角色、工具白名单、Review Profile 注册与业务 binding、`runtime.enforcement` 的权威配置；启动期失败关闭，修改必须通过契约和门禁测试 |
...
| Agent role contract | `config/agents.yaml` | 不属于 Codex 官方 Agent 声明；由 NovelOS MCP 读取并验证角色、Profile 引用和 enforcement。默认 `strict_isolation_evidence=false`、`strict_cross_consistency=false`，切换 strict 前必须运行对应边界测试 |
