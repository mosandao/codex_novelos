...
3. 方法论获取（按资产分流）：**已模块化资产**（`direction`）不 Read prompt.md——组装器 `--asset direction` 一步产出「主干 + 条件模块 + 输入数据区 + 自检汇总」完整注入文本（channel/platform/genre/aesthetic 随 setup 自动路由，无需主控显式传入）；**未模块化资产**（architecture / strategy / character_contract / world_contract / story_arc / volume_outline / chapter_plan）暂仍 Read `catalog/skills/planning/<对应 skill>/prompt.md`，Task 29 P2 完成后逐一切换为组装器。
4. 探索性讨论直接返回方案，不持久化。
