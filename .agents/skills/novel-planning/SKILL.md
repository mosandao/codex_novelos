...
通过 node:sqlite 只读查询数据库；写路径唯一入口 = 插件六个门工具（见 AGENTS.md「数据库访问」）。SQL 模板见 `novel-project/sql-reference.md`。
