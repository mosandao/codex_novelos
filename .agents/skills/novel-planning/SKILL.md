...
通过 node:sqlite 操作数据库（读=只读查询；写=受控直写，锁定/审查落库走状态机纪律，见 AGENTS.md「数据库访问」）。SQL 模板见 `novel-project/sql-reference.md`。
...
升级裁决可见性：任一上游资产处于升级用户裁决状态（adjudications open 行）时，其 lock/accept 被门互锁阻断，且章纲/规划组装经 `open_adjudications` 槽显式可见该未决清单——地基未定的 subject 禁止当作已定事实消费，规划侧须在候选中显式声明对未决资产的假设或等待裁决。
...
6. sub agent 返回候选后落库为**候选**（仅允许 `status='candidate'`；`locked`/`stale`/`superseded` 状态变更按第 8 步与「上游变更与 stale 传播」纪律执行）：
   ...
   ```
7. 用 `$novel-review` 审查（sub agent 审查 → 主控按 sql-reference.md「审查」模板以受控 SQL 落回执：`subjectRef`+`verdict`+`findingsJson`+`reviewerProfile`（格式 `model:<provider:model>` 或 `agent:<name>@<model>`，reviewer_profile 带身份前缀））。direction 的审查 rubric 同样按项目组装：`node scripts/novelos-compose-prompt.mjs --asset direction-review --project <project_id> --subject <候选资产ID>`——频道语法/平台画像/题材信息包/美学基因的专项检查随项目路由，与生成端对称。direction 产出 2-3 候选时按 novel-review「横向回执」汇总比较呈报用户，用户裁决选定后仅对选定候选走锁定循环。
8. 审查通过后锁定（单事务，受控 SQL）：① 先核对绑定回执存在且 `verdict='approved'` 且 `subject_ref`=该资产——跳审锁定禁止；② 先把同 scope 旧 locked 行翻 `superseded` 再置本行 `locked`（部分唯一索引 `idx_planning_assets_current`，先 supersede 后 lock）；③ 直接上游未全部 locked 时按依赖顺序拒绝。SQL 模板见 sql-reference.md「规划资产」。**character_contract 锁定后**：主控以受控 SQL 落人物注册表（characters 表：main/secondary + arc_role/预期退场/登场卷/seat_ref/essence 随 state_json；席位对账 + 近重名 WARN）。**volume_outline 锁定后**：若候选 metadata 带 `volume_characters`（卷级配角班底，secondary/minor），主控随即落人物注册表（条目带 source:"volume_outline"、arc_role、预期退场、来源卷、seat_ref；席位对账：seat_ref 引用不存在 = FAIL，未认领承诺席位 WARN 终核）；随后只读终核班底落表（漏跑即报告非空）；`volume_settings` 中 disposition=登记入world 的条目走 world change proposal 增补，`new_plants`/`drift` 经 change proposal 增补 story_arc 台账与映射表。
9. 若下游 Agent 发现上游问题，返回变更提案由主控路由给上游所有者，不在下游候选中隐式重写上游。

## 上游变更与 stale 传播

上游资产修订（新 revision locked）后，主控以受控 SQL 沿 `planning_asset_dependencies` 依赖边把下游标 `stale`（模板见 sql-reference.md「规划资产」）。
...
| `cadence_plan`（可选） | object | 兑现规划：{fulfillment_count, interval_volumes, notes?}——`--scale` 档位数字规则（短篇 1-2 / 中篇 ≥3 / 长篇 ≥3 / 超长篇 ≥5）；新 direction 候选必带 |

校验口径：结构 + lineage 覆盖 + cadence_plan 数字规则，按本表逐项对照自查（对照 `config/schemas/book-soul.schema.json`）。book_soul 只有 v2 一个版本；既有 v1 资产属历史锁定数据，不参与新候选校验。
...
校验口径：本表逐项对照自查（对照 `config/schemas/architecture-metadata.schema.json`）——tier 与 beats 一致性、空窗上限×scale 档位、油耗分级下限与 book_soul cadence 数字规则同源。低密度主线（柯南/X 档案式空窗+爆发）合法，前提是空窗有上限、爆发点有位置设计、与上游对表论证。
...
| 字段 | 类型 | 约束 |
|---|---|---|
| `consumption[]` | object[] | 恰 7 行上游消费翻译：{output（rhythm_table/reveal_ladder/promise_cadence/power_escalation/spiral_rotation/engine_config/upstream_receipts）, translation, ref}——缺行 = 上游静默蒸发，自查拦截 |
...
| `pairing_cycle` | object | {debt_streak_limit: 1-3}——连续纯存债阶段上限（自查核验实际连续段） |
...
校验口径：结构 + 七行消费覆盖 + 阶段数×档位区间[短篇 1-2/中篇 2-4/长篇 3-8/超长篇 5-12] + 存债连续 + 中盘续命 + 收束预算/终局字数下限，按本表逐项对照自查（对照 `config/schemas/strategy-metadata.schema.json`）。
...
| `dimension_costs[]` | object[] | 1-12 维度：{dimension, form, reversibility（可逆/压制/不可逆）, threshold?, release?, bearer?, book_soul_ref?}——压制必带 release；不可逆必带 threshold（自查复核）；bearer=protagonist_permanent 必带 book_soul_ref（世界层不得新增主角永久代价） |
| `decision_points[]` | object[] | 0-4 条世界层命门决策点；无关可省略 |

校验口径：schema + 岗位重名 + 代价两轴，按本表对照自查（对照 `config/schemas/world-metadata.schema.json`）。character 侧 roster 规模档位：短篇 2-5 / 中篇 3-8 / 长篇 5-12 / 超长篇 8-16；roster 校验含规模门 + 席位对账——scale/locked world 从库解析，主控落注册表时执行（无独立机器门）。
...
| 字段 | 类型 | 约束 |
|---|---|---|
| `volume_plan[]` | object[] | 1-40 卷：{index, word_range{min,max}}——**全书卷数权威在此声明**（strategy 只给阶段字数，卷切分归弧层）；卷字数总和与阶段字数同量级（自查对表） |
| `arcs[]` | object[] | 短篇 1-2/中篇 2-3/长篇 3-5/超长篇 5-7（自查 ×scale）；主线（kind=主线）恰 1 条；{arc_id slug, name, kind（主线/人物/关系/世界/对手/其他）, carriers[{ref, ref_type: roster/seat/latent}], start_state, end_state, turning_points[]}——主线/人物/关系弧必须具名载体；弧首活跃卷 ≥ 载体登场卷（自查对账） |
| `arc_volume_map[]` | object[] | 行式映射表 {arc_id, volume, duty（推进/蓄势/兑现/收束/休眠）, note}——每卷 ≥1 推进（error）/ 推进 ≤2（warn）/ 同时活跃（推进/兑现/收束）≤4（error）/ 禁全推进全休眠（自查） |
| `plant_payoff_ledger[]` | object[] | {line_id, claim, source_type（book_soul/strategy/arc）+source_ref, plant_volume, partial_payoffs[], close_volume+close_form（兑现/违约/转化）XOR exempt}——收束与豁免二选一；合法豁免仅 deliberate_silences 与 open 喂料；卷 2 起每卷至少兑现一条（自查） |
| `variation_alloc[]` | object[] | {test_ref（引用 book_soul recurring_tests 编号）, volume, changed[]（处境/答案/代价至少其一）, mech_ref（architecture 机制名——自查存在性对账）, note}；同一母题 >3 次变奏 warn（剩余空间评估） |
| `open_window` | object | strategy terminal_mode=open 必填（自查）：{hard_volumes 1-5, note}——近卷硬格/远卷软格滚动窗口；**增量修订从 arc_states 实际进展出发，写偏的弧是已发生事实** |
| `decision_points[]` | object[] | 0-4 条弧层命门决策点（弧数取舍/终点形态/种收节奏）；无关可空数组，不凑数 |

校验口径：scale 与 character/world/architecture/strategy 四上游 locked metadata 从库解析，按本表对照自查（对照 `config/schemas/story-arc-metadata.schema.json`）。连续性侧 `arc_states.arc_ref` 引用 `arc_id` slug（无对应弧用描述性 key，不发明新 id——planned vs actual 对账依赖此约定）。
...
| 字段 | 类型 | 约束 |
|---|---|---|
| `volume_number` | int | 卷号锚定——前置 locked 卷须为 1..N-1 连续，本卷 = N（自查核验，prev 卷链按卷号注入）；乱序规划在此拦截 |
| `word_range` | object | {min, target, max} 本卷字数申报——与 story_arc volume_plan 本卷行有交集（自查）；target 是副高湂数条件化基准 |
...
| `mainline_beats` | int | 0-8——对架构 beats_per_volume ±2（自查）；tier=低 允许 0 拍卷 |
| `climax_positions[]` | number[] | 卷长比例升序，末位 = 1（卷末主高潮）；相邻间距 × target ≤ 30 万字；target ≥ 20 万时总数 ≥ target÷25万 向上取整；短篇退化（<20 万字）允许仅主高潮（自查） |
...
| `new_plants[]` | object[] | 本卷新种悬念 {line_id, claim, close_volume XOR exempt}——收束卷 ≥ 本卷、不与既有台账冲突（自查）；主控据此经 change proposal 增补台账；终卷新种不得溢出终卷 |
| `drift[]` | object[] | 计划 vs 实际漂移 {arc_id, planned, actual, proposal}——arc_id 须存在（自查）；以实际结算为优先源 |
| `test_alloc[]` | object[] | 本卷变奏承接 {test_ref, changed[]}——与 variation_alloc 本卷行双向对账（漏承接/超分配均 warn，自查） |
...
| `volume_characters[]` | object[] | 班底（planning-candidate $defs 同构，secondary/minor）——锁定后主控以受控 SQL 落人物注册表，随后只读终核漏跑 |

校验口径：scale 与 story_arc/architecture/strategy locked metadata + 前置卷号 + 注册表名册从库解析，按本表逐项对照自查（对照 `config/schemas/volume-outline-metadata.schema.json`）。

## character essence 速查表（T37）

roster 行的 `essence`（main 必填，≤160 字）：执念/失稳点/语域口癖/最要命的关系——「写他时必须抓住什么」。契约锁定后随受控 SQL 落注册表 state_json，写作端与 prose-review 经 **character_essence 槽**按行消费（出场人物卡：要点 + 死活状态；已退场人物不得无连续性依据出场）。二级造人端（卷纲班底/执行卡微档案）另经 **persona_gate 槽**注入分身硬边界（cannot_write/refuses/表达偏好/负向约束）——新造人物不得整档落在盲区场景，确需涉盲区者微档案带绕开方式。

## 节奏密度约束

战略骨架（Strategy）的阶段数按 scale 档位区间约束（短篇 1-2 / 中篇 2-4 / 长篇 3-8 / 超长篇 5-12，按速查表自查），阶段判据以事件（不可逆变更 + 螺旋轮换）为主，字数区间为申报项。节奏密度在 Volume Outline 及以下实现。Volume Outline 必须产出并行冲突线（每卷 ≥3 条，跨卷弧切片与卷内自含线双 scope）、高潮位置按**本卷字数**条件化（相邻间距 ≤30 万字、target ≥20 万时总数 ≥ target÷25万 向上取整、短篇退化允许仅卷末主高潮——按速查表自查）、POV 多样性（频道轴 POV 配置在条件模块）。
