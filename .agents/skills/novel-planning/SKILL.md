...
用 `scripts/novelos_validate_story_arc.py metadata.json --project <project_id>` 校验（scale 与 character/world/architecture/strategy 四上游 locked metadata 自动解析，也可显式 `--scale/--character/--world/--architecture/--strategy`）。连续性侧 `arc_states.arc_ref` 引用 `arc_id` slug（无对应弧用描述性 key，不发明新 id——planned vs actual 对账依赖此约定）。
...
用 `scripts/novelos_validate_volume_outline.py metadata.json --project <project_id>` 校验（scale 与 story_arc/architecture/strategy locked metadata + 前置卷号 + 注册表名册自动解析，也可显式 `--scale/--story-arc/--architecture/--strategy`）。
