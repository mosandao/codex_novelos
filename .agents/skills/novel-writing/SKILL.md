...
2. Creator Profile 签名含 `persona`（创作者人格）时必须注入 Writer prompt：narrative 全文 + anchors（目光/五维/内在矛盾/声音样本/盲区）。Writer 写到超出这位作者经验边界的场景时，按 persona 的方式处理——绕开、转喻、有限视角（`blindspots.cannot_write` 列出的圈子尤其如此），**禁止切换全知叙述假装在场**。
