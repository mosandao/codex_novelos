...
用法::
    ...
    python legacy-python/scripts/novelos_export_kernel_roster.py            # 写入 ui/kernel-roster.js
    python legacy-python/scripts/novelos_export_kernel_roster.py --check    # 校验镜像与库一致
"""
...
ROSTER_FILE = REPO_ROOT / "plugin" / "client" / "kernel-roster.js"

HEADER = "// 由 legacy-python/scripts/novelos_export_kernel_roster.py 生成——请勿手改；建核/修订内核后重跑刷新。\n"
...
def main() -> int:
    ...
    if args.check:
        ...
        if current != content:
            print("kernel-roster.js 与库不一致——重跑 legacy-python/scripts/novelos_export_kernel_roster.py 刷新")
            return 1
