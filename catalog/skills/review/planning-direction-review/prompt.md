...
5. **规模数字门**：`project_setup.scale` 档位的兑现次数与间隔必须匹配——短篇完整兑现 1-2 次、中篇 3 次量级间隔 ≤30 万字、长篇 3-4 幂兑现间隔 ≤80 万字、超长篇 ≥5 次间隔 ≤100 万字。优先核 `book_soul.cadence_plan`（次数下限对照档位规则自查，机器门未覆盖），并查 promise_cadence 文本与 cadence_plan 一致；新候选缺 cadence_plan 或文本与计划矛盾 = warning；既无 cadence_plan 且文本含糊到无法核定兑现次数 = blocking，并注明移交 volume_outline 审查回查实际间隔。
