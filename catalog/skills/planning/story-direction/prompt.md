...
## 上游消费：表里基调、题材信息包与平台耐心

`project_setup` 的以下字段是**硬输入**，direction 必须显式消费（主控从 `projects.metadata_json` 的 `$.setup` 读取后注入）：
...
- **`genre_profile`（题材信息包）非空时**：`power_currency_candidates` 是力量货币候选池——从中选取或组合，**不现场发明**；`typical_dilemmas` 是 central_contradiction 候选方向；`reader_expectations` 校准 costly_commitments（读者等待的承诺必须有对应物）；`taboos` 进禁止方向。**为 null 时**：现场推导力量货币并在 direction 正文显式定义（谁发行、怎么挣、怎么贬值），标准不降。
- **`platform_traits`（平台画像）决定 promise_cadence 的节奏翻译**：`patience` 是平台的注意力预算——「开篇即冲突、前三章定生死」→ 第一章内必须首个可感承诺兑现、前 3 章完成首次完整兑现循环；「付费订阅养书容忍」→ 前三十章可缓坡蓄势，但每章须有增量。direction 的 promise_cadence 必须引用平台耐心并给出对应节奏，不许写与平台画像脱节的通用节奏。
...
**好坏对照**（合成示例）：坏的核心矛盾「主角既要变强又要守护伙伴」（无两难，可同时成立）；好的「他必须练成仇人的手艺才能复仇，而这门手艺每精进一分，就把他变成仇人一分」（两个判断互斥，都有代价）。坏的组织原则「热血升级流」（任何书都成立）；好的「每一场硬仗都在一张谈判桌上完成，胜负在茶凉之前」（绑定本书的场景机制，换一本书即不成立）。
