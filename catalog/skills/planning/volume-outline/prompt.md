...
候选 metadata 符合 `config/schemas/volume-outline-metadata.schema.json`（v1）：`volume_number`（卷号锚定）/ `word_range{min,target,max}`（对 volume_plan 本卷行）/ `volume_form` + `units`/`exit_settlement`（卷型件）/ `lines[]`（name/scope/arc_id/share_pct/pov/note，mainline 0-1 条）/ `mainline_beats` / `climax_positions` / `new_plants[]` / `drift[]` / `test_alloc[]` / `volume_settings[]` / `volume_characters`（planning-candidate $defs 同构）。交付前过 `scripts/novelos_validate_volume_outline.py metadata.json --project <project_id>`（卷号连续/字数对表/高潮门/线弧双向/单元编排/换图清算/终卷纪律/班底预检；也可显式 `--scale/--story-arc/--architecture/--strategy`）。
