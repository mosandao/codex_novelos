...
- **内核层继承不变**：分身的核心问题感、价值排序、心理运作（八维）、知识边界必须能追溯到内核的 identity / psychology / knowledge_ecology——语义继承、重新长出，**禁止逐字复制内核条目**（逐字比对由**主控自查程序**执行：派生产出后，主控对 core_questions / value_axioms / aesthetic_commitments / creative_axioms 四组逐条与注入内核原文做子串比对，逐字命中 ≥1 条即打回重融合；注意 emotional_stance.sympathies 与签名 sympathies 同名同义，是最高复制风险区，一并比对）。「重新长出」的合格判据：签名条目须含本书语境词（题材/人物/场景锚点），纯换词复读按复制处理。
